...
from models.resnet import resnet50, resnet10, resnet50c, resnet18c
...
class HopeNet2(nn.Module):

    def __init__(self):
        super(HopeNet2, self).__init__()
        self.resnet = resnet18c(pretrained=False, num_classes=29*2)
        self.graphnet = GraphNet(in_features=1026, out_features=2)
        self.graphunet = GraphUNet(in_features=2, out_features=3)

    def forward(self, x, y):
        points2D_init, features = self.resnet(x, y)
        features = features.unsqueeze(1).repeat(1, 29, 1)
        # batch = points2D.shape[0]
        in_features = torch.cat([points2D_init, features], dim=2)
        points2D = self.graphnet(in_features)
